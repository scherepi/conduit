...
import MessageParser, { decodeMessage, encodeMessage, MESSAGE_TYPE, PORT_STATUS } from "./messages.ts";
const cli = meow(
    ...
    conduitSocket = await Bun.connect({
        ...
        socket: {
                ...
                for (const parsedMessage of parser.parseMessages()) {
                    switch (parsedMessage.messageType) {
                        case MESSAGE_TYPE.DATA:
                            localTunnels[parsedMessage.connectionId]?.write(parsedMessage.payload || new Uint8Array());
                            break;
                        ...
                        case MESSAGE_TYPE.PORT_RESPONSE:
                            ...
                            if (portStatus == PORT_STATUS.SUCCESS) {
                                ...
                                console.log(`You (or your friends) should be able to access it at ${hostname}:${assignedPort}`);
                                ...
                                console.log("Try running the command without specifying a remote port - the server will assign you what's open.");
                            }
                            break;
                        case MESSAGE_TYPE.PORT_ASSIGNED:
                            // unpack the Uint8Array to a port number
                            assignedPort = ((parsedMessage.payload?.[0] ?? 0) << 8) | (parsedMessage.payload?.[1] ?? 0); // dude what does this even mean
                            ...
                            if (assignedPort == 0 || isNaN(assignedPort)) {
                                ...
                                console.error("This shouldn't happen - please leave an issue on the GitHub Repository");
                                console.log("Bugs are an important part of the ecosystem ✨");
                                break;
                            }
                            console.log(`Successfully connected to conduit server. You've been assigned port ${assignedPort}.`);
                            console.log(`Tell your friends to visit ${hostname}:${assignedPort} to see your work!`);
                            break;
            ...
            open(socket) {
                ...
                if (remotePort) {
                    const portRequestMessage = encodeMessage(0, MESSAGE_TYPE.PORT_REQUEST, new Uint8Array([remotePort >> 8, remotePort & 0xFF]));
            ...
            connectError(_socket, _error) {
                ...
                console.error("Failed to connect to the conduit server. Please check your network connection and the hostname.");
            },
    ...
    localTunnels[connectionId] = await Bun.connect({
        ...
        socket: {
            ...
            error(_socket, _error) {
                ...
                console.error("You shouldn't be seeing this - please make an issue on the GitHub repository.");
                console.log("Bugs are how flowers grow ✨");
            },

            connectError(_socket, _error) {
                console.error("Something went wrong establishing the local tunnel.");
                console.log("You shouldn't be seeing this - please make an issue on the GitHub repository.");
                console.log("Bugs are how flowers grow ✨");
                const encodedMessage = encodeMessage(connectionId, MESSAGE_TYPE.CONNECTION_CLOSED, null);
                conduitSocket?.write(encodedMessage);
                delete localTunnels[connectionId];
            ...
            timeout(_socket) {
                ...
                console.log("You shouldn't be seeing this - please make an issue on the GitHub repository.");
                console.log("Cool bug fact: bombardier beetles can eject boiling chemical spray from their abdomen when threatened!");
            }
        },
    });
}
